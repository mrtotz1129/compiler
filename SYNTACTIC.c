...
Tree* SYNTACTIC(){
    ...
    fileline = count = errorCount = adLine = lvl = tmplvl = y = isOneSmt = 0;
...
void error(){
    ...
    synlen = synlen + strlen(errorMessage);
    setSyn(1, synlen);
...
void SMT(){
    ...
    if(isOneSmt){
        ...
        else if(isMatch("st_cont")||isMatch("st_break")){
            current = tempCurr;
            if(setCode("st_break")
            ||setCode("st_cont")){
                insertNode(current, token, current->level + 1, line, 1);
...
Tree* newNode(char* data, int level, int line) {
    ...
    if(!strcmp(data, "%i")) strcpy(data, "%%i");
    else if(!strcmp(data, "%s")) strcpy(data, "%%s");
    else if(!strcmp(data, "%c")) strcpy(data, "%%c");
    else if(!strcmp(data, "%b")) strcpy(data, "%%b");
    else if(!strcmp(data, "%f")) strcpy(data, "%%f");
