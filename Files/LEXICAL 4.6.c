        ...
        if(x->character == '\n'||x->character == ' '||x->character == '\t'){
            ...
            switch(x->character)
            {
                    ...
                    if(x!=tail&&x->next->character == '+'){
                        ...
                        if(isdigit(x->next->character) && !(isalnum(x->prev->character))){
                            for(y = x->next; y->character!=' '&&y!=NULL;y=y->next){
                                if(y->prev->prev->character=='=' || y->prev->prev->prev->character=='='){
                                        neg=1;
                                    break;
                                }
                ...
                case '-':{
                    ...
                    else if((x->prev->prev->character==' ' && !(isalnum(x->prev->prev->prev->character)))
                    || (!(isdigit(x->prev->prev->character)))){
                        if(isdigit(x->next->character) && !(isalnum(x->prev->character))){
                            for(y = x->next; y->character!=' '&&y!=NULL;y=y->next){
                                if(y->prev->prev->character=='=' || y->prev->prev->prev->character=='='){
                                ...
                                else{ neg=1;fprintf(Symbol, "%c",y->character);
                ...
                case '(':{
                    ...
                    decim = 0;
                    neg=0;
                    if(x->next->character=='-'||x->next->character=='+'){//printf("pass0---\n");
                       // neg = 1;
                        for(y = x->next; y->character!=')'&&y!=NULL;y=y->next){
