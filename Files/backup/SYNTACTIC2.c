...
int logParen = 0, relParen = 0, arithParen = 0, templine = 0;
...
void SNS(){
    fprintf(PT, "SNS -> ");
    if(setCode("meth_main")){ fprintf(PT, " meth_main"); getNextToken();} else error();
    if(setCode("oCurl")){ fprintf(PT," %s( %s )", token, lexeme); getNextToken();} else error();

    SMT();

    if(fileline==count){
        if(setCode("cCurl")){ fprintf(PT," %s( %s )", token, lexeme); getNextToken();}
    }else error();
...
void SMT(){
    strcpy(stmt,"");
    fprintf(PT, " SMT( ");
    if(DATA_TYPE()){fprintf(PT, "DEC_SMT( ");  DEC_SMT();}
    else if(isMatch("st_read")){fprintf(PT, "INPUT_SMT( "); INPUT_SMT();}
    else if(isMatch("st_print")){fprintf(PT, "OUTPUT_SMT( "); OUTPUT_SMT();}
    else if(isMatch("ident")||isMatch("inval_ident")){fprintf(PT, "ASSIGN_SMT( "); ASSIGN_SMT();}
    else if(isMatch("lp_while")||isMatch("lp_do")||isMatch("lp_for")){fprintf(PT, "LOOP_SMT( "); LOOP_SMT();}
    else if(isMatch("cond_if")){fprintf(PT, "COND_SMT( "); COND_SMT();}
    else if(isMatch("cond_swt")){fprintf(PT, "SWITCH_SMT( "); SWITCH_SMT();}
    else if(SYMBOLS()) error();
    else return;

    if(!isMatch("cCurl")){fprintf(PT, " )\n"); SMT();}
}//SMT()

void DEC_SMT(){
    strcpy(stmt,"declaration");
    if(DATA_TYPE()){ fprintf(PT, "DATA_TYPE ( %s( %s ) )", token, lexeme );getNextToken();}

    if(isMatch("inval_ident")||setCode("ident")){
        if(setCode("inval_ident")) error();
        fprintf(PT," %s( %s )", token, lexeme);
        getNextToken();

        if(setCode("term_sym")){fprintf(PT," %s( %s ) )", token, lexeme); getNextToken();}
            ...
            if(EXPR()){
                ...
                if(setCode("inval_ident"))error();
                else fprintf(PT," %s( %s ) )", token, lexeme);
                getNextToken();
            }
            else error();
        }
...
void INPUT_SMT(){
    strcpy(stmt,"input");
    if(isMatch("st_read")){fprintf(PT," %s( %s )", token, lexeme);  getNextToken();}
    if(setCode("ident")){fprintf(PT," %s( %s )", token, lexeme); getNextToken();} else error();
    if(setCode("term_sym")){fprintf(PT," %s( %s ) )", token, lexeme); getNextToken();} else error();
}//INPUT_SMT()

void OUTPUT_SMT(){
    strcpy(stmt,"output");
    if(isMatch("st_print")){fprintf(PT," %s( %s )", token, lexeme); getNextToken();}

    if(setCode("ident")){fprintf(PT," %s( %s )", token, lexeme); getNextToken();}
    else if(setCode("val_str")){fprintf(PT," %s( %s )", token, lexeme); getNextToken();}
    else error();
    if(setCode("ccat_out")){
        fprintf(PT," %s( %s )", token, lexeme);
        getNextToken();
    ...
    else if(setCode("term_sym")){fprintf(PT," %s( %s ) )", token, lexeme); getNextToken();}
    else error();
}//OUTPUT_SMT()
...
void LOOP_SMT(){
    if(isMatch("lp_while")){ fprintf(PT," WHILE_SMT("); WHILE_SMT();}
    else if(isMatch("lp_do")){ fprintf(PT," DO_SMT("); DO_SMT();}
    else if(isMatch("lp_for")){ fprintf(PT," FOR_SMT(");FOR_SMT();}
}

void FOR_SMT(){
    if(isMatch("lp_for")){ fprintf(PT," %s( %s )", token, lexeme); getNextToken();}

    if(setCode("oParen")){
        fprintf(PT," %s( %s )", token, lexeme);
        getNextToken();
        if(DATA_TYPE()){fprintf(PT," %s( %s )", token, lexeme); getNextToken();}
        ASSIGN_SMT();
        ...
        if(setCode("cParen")){fprintf(PT," %s( %s )", token, lexeme); getNextToken();}
        else error();
    }else error();
...
void DO_SMT(){
    if(isMatch("lp_do")){ fprintf(PT,"  %s( %s )", token, lexeme); getNextToken();}
    if(setCode("oCurl")){
        getNextToken();
        SMT();
        if(setCode("cCurl")){
...
int CONST(){
    if(setCode("val_int")||setCode("val_float")||setCode("val_str")||
       ...
       if(!strcmp(token,"inval_sym")||
        ...
        else fprintf(PT," CONST( %s( %s ) )", token, lexeme);
        return 1;
    }
    return 0;
}//CONST()
