...
static GtkTextTag *tag1;
static GtkTextTag *tag2;
static GtkTextTag *tag3;
static GtkTextIter start, end;
int lex = 0 , syn = 0, sem = 0;
int lexLength = 0, synLength = 0, semLength = 0;
...
void open(){
    ...
    if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT)
    {
           ...
           && filename[strlen(filename)-4] == '.')
        {
            ...
            messagebox = gtk_message_dialog_new (window,
...
void compile(){
    ...
    if(lex == 1){
        gtk_text_buffer_get_iter_at_offset (MessageBuffer, &start, 0 );
        gtk_text_buffer_get_iter_at_offset (MessageBuffer, &end, lexLength);
        gtk_text_buffer_apply_tag (MessageBuffer, tag1, &start, &end);
    }
    if(syn == 1){
        gtk_text_buffer_get_iter_at_offset (MessageBuffer, &start, lexLength );
        gtk_text_buffer_get_iter_at_offset (MessageBuffer, &end, (synLength+lexLength));
        gtk_text_buffer_apply_tag (MessageBuffer, tag2, &start, &end);
    }
    if(sem == 1){
        gtk_text_buffer_get_iter_at_offset (MessageBuffer, &start, (synLength+lexLength));
        gtk_text_buffer_get_iter_at_offset (MessageBuffer, &end, (semLength+synLength+lexLength));
        gtk_text_buffer_apply_tag (MessageBuffer, tag3, &start, &end);
    }
}

void setLex(int x, int y){
    lex = x;
    lexLength =y;
}

void setSyn(int x, int y){
    syn = x;
    synLength = y;
}
void setSem(int x, int y){
    sem = x;
    semLength = y;
}
...
static void update_statusbar (GtkTextBuffer *buffer, GtkStatusbar  *statusbar){
    ...
    gtk_statusbar_pop (statusbar, 0); // clear any previous mess    age, underflow is allowed
...
GtkWidget* initGUI(int argc, char** argv){
    ...
    NewImage = gtk_image_new_from_file("image/new.png");
    NewButton = gtk_tool_button_new(NewImage, "SaveAs");
    ...
    SaveAsImage = gtk_image_new_from_file("image/saveas.png");
    SaveAsButton = gtk_tool_button_new(SaveAsImage, "SaveAs");
    ...
    gtk_table_attach_defaults(GTK_TABLE(table), notebook, 0,8,5,7);
    gtk_widget_show(notebook);

    label = gtk_label_new("Message");
    gtk_notebook_append_page(GTK_NOTEBOOK (notebook), MessageFrame, label);
    label = gtk_label_new("Output");
    gtk_notebook_append_page (GTK_NOTEBOOK (notebook), OutputFrame, label);
    ...
    color.red   = 0x00C0 * 65535/255;
    color.green = 0x00DE * 65535/255;
    color.blue  = 0x00ED * 65535/255;
    gtk_widget_modify_bg(window, GTK_STATE_NORMAL, &color);
    gtk_widget_modify_bg(notebook, GTK_STATE_NORMAL, &color);
    gtk_widget_modify_bg(MenuBar, GTK_STATE_NORMAL, &color);
    gtk_widget_modify_bg(FileMenu, GTK_STATE_NORMAL, &color);
    gtk_widget_modify_bg(CompileMenu, GTK_STATE_NORMAL, &color);
    color.red = 65535;
    color.green = 65535;
    color.blue = 65535;
    gtk_widget_modify_bg(MenuToolBar, GTK_STATE_NORMAL, &color);
    gtk_widget_modify_base(MenuToolBar, GTK_STATE_NORMAL, &color);

    //Set Font
    font_desc = pango_font_description_from_string ("Consolas");
    gtk_widget_modify_font (EditorText, font_desc);
    pango_font_description_free (font_desc);

    font_desc = pango_font_description_from_string ("Calibri");
    gtk_widget_modify_font (MessageText, font_desc);
    gtk_widget_modify_font (OutputText, font_desc);
    pango_font_description_free (font_desc);
    //Left Margin
    gtk_text_view_set_left_margin (GTK_TEXT_VIEW (EditorText), 5);
    gtk_text_view_set_left_margin (GTK_TEXT_VIEW (MessageText), 10);
    gtk_text_view_set_left_margin (GTK_TEXT_VIEW (OutputText), 5);
    //Foreground
    gdk_color_parse ("white", &color);
    gtk_widget_modify_text (OutputText, GTK_STATE_NORMAL, &color);
    gdk_color_parse("black", &color);
    gtk_widget_modify_base(OutputText, GTK_STATE_NORMAL, &color);

    tag1 = gtk_text_buffer_create_tag (MessageBuffer, "green_foreground", "foreground", "green", NULL);
    tag2 = gtk_text_buffer_create_tag (MessageBuffer, "red_foreground", "foreground", "red", NULL);
    tag3 = gtk_text_buffer_create_tag (MessageBuffer, "blue_foreground", "foreground", "blue", NULL);
