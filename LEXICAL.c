...
int ln = 1, state = 0, str = 0, decim = 0, key2 = 0, key = 0, eCount = 0, neg = 0, lexlen = 0;
char tempLexeme[50];
...
int LEXICAL(char file[], struct token *tokenHead){
    ln = 1;
    state = str = decim = key2 =  key =  neg = eCount =0;
    lexlen = 0;
    head = tail = curr = NULL;
    tHead = tokenHead;
...
void Output(char token[]){
    Symbol = fopen("SymbolTable.sns","aw+");
    char *errorMessage = malloc(70);
    fprintf(Symbol, "\t\t\t %s\t\t %d\t\t\t %d\n", token, state, ln);
    if(!strcmp(token, "inval_num")
       ||!strcmp(token, "inval_sym")
       ||!strcmp(token, "inval_ident")
       ||!strcmp(token, "inval_str")
       ||!strcmp(token, "inval_char")
       ||!strcmp(token, "inval_math")
       ||!strcmp(token, "inval_log")){
        eCount++;
        sprintf(errorMessage, "<line %d> -- Lexical Error < %s >\n", ln, token);
        Message(errorMessage);
        lexlen = lexlen + strlen(errorMessage);
        setLex(1,lexlen);
    }
